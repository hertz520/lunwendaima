...
noise_levels = [0.20, 0.25]

for noise_level in noise_levels:
    noisy_data = add_gaussian_noise(X, noise_level)
    accuracies = []

    for train_index, test_index in kf.split(noisy_data):
        X_train, X_test = noisy_data[train_index], noisy_data[test_index]
    ...
    print(f'平均准确率 ({int(noise_level*100)}% 高斯噪声): {np.mean(accuracies):.4f}')
