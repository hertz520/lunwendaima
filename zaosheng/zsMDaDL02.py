...
def add_gaussian_noise(data, noise_level):
    ...
    noise = noise_level * np.random.normal(size=data.shape) * std
    noisy_data = data + noise
    return noisy_data

def add_noise_with_factor(data, noise_factor):
    X_bar = np.random.normal(size=data.shape)
    lambda_factor = noise_factor * np.linalg.norm(data, 'fro') / np.linalg.norm(X_bar, 'fro')
    noisy_data = data + lambda_factor * X_bar
    return noisy_data

kf = KFold(n_splits=10, shuffle=True, random_state=42)

noisy_data_20 = add_gaussian_noise(X, 0.20)

noise_factors = [0.1, 0.2, 0.3, 0.4]

for noise_factor in noise_factors:
    noisy_data_with_factor = add_noise_with_factor(noisy_data_20, noise_factor)
    accuracies = []

    for train_index, test_index in kf.split(noisy_data_with_factor):
        X_train, X_test = noisy_data_with_factor[train_index], noisy_data_with_factor[test_index]
    ...
    print(f'平均准确率 (20% 噪声 + 噪声因子 {noise_factor}): {np.mean(accuracies):.4f}')
